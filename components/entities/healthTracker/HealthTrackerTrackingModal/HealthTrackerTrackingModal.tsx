...
import { getFieldConfig, validateHealthTrackerInput } from "./utils";
...
> = ({ healthTracker, onClose }) => {
  ...
  const isFormValid = useCallback(() => {
    // For menstrual cycle, always valid if toggle is set
    if (healthTracker.type === HealthTrackerTypes.MENSTRUAL_CYCLE) {
      return true;
    }
  ...
  const handleValue1Change = useCallback((text: string) => {
    setValue1(text);
    setErrors([]);
  }, []);

  const handleValue2Change = useCallback((text: string) => {
    setValue2(text);
    setErrors([]);
  }, []);
  ...
  const handleRecord = useCallback(async () => {
    ...
    setTimeout(async () => {
      // Validate inputs (skip validation for menstrual cycle since we use toggle)
      if (healthTracker.type !== HealthTrackerTypes.MENSTRUAL_CYCLE) {
        const validation = validateHealthTrackerInput(
          healthTracker.type,
          value1,
      ...
      try {
        const data: HealthTrackingLogDataForInsert = {
          date: new Date(),
          value1:
            healthTracker.type === HealthTrackerTypes.MENSTRUAL_CYCLE
              ? toggleValue
                ? 1
                : 0
              : parseFloat(value1),
  ...
  // Custom title for menstrual cycle
  const getModalTitle = () => {
    if (healthTracker.type === HealthTrackerTypes.MENSTRUAL_CYCLE) {
      return `${getHealthTrackerEmoji(healthTracker.type)} ${LanguageService.translate("Did you have cycle today?")}`;
    }
    return `${getHealthTrackerEmoji(healthTracker.type)} ${getHealthTrackerName(healthTracker.type)} ${LanguageService.translate("Now").toLowerCase()}`;
  };
          ...
          {healthTracker.type === HealthTrackerTypes.MENSTRUAL_CYCLE ? (
            <>
              <Toggle
                ...
                label={LanguageService.translate(fieldConfig.label1Key)}
                error={errors.length > 0 ? errors[0] : undefined}
              />
              {/* Temporary debug display */}
              <Text style={{ marginTop: 10, fontSize: 12, color: "#666" }}>
                Debug: Toggle value = {toggleValue.toString()}
              </Text>
            </>
          ) : (
            <>
